...
lookupBy' :: forall k v. Ord k => (k -> Boolean) -> Map k v -> Maybe (Tuple k v)
lookupBy' f = Map.filterKeys f >>> Map.toUnfoldable >>> List.head


lookupByEq :: forall a k v. Ord k => Eq a => (k -> a) -> a -> Map k v -> Maybe v
lookupByEq f sample = lookupBy (f >>> (==) sample)


lookupByEq' :: forall a k v. Ord k => Eq a => (k -> a) -> a -> Map k v -> Maybe (Tuple k v)
lookupByEq' f sample = lookupBy' (f >>> (==) sample)
